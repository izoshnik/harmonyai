...
export default async function handler(req, res) {
  ...
  try {
    /* memoryFromClient=true — память уже вшита в system-prompt клиентом
       (js/hm-memory.js). Тогда отдельный поход в Supabase за user_memories
       не нужен: меньше запросов к БД на каждое сообщение. Флаг нет → старое
       поведение, ничего не ломается. */
    let { messages, model, userId, think = false, effort = 'low', stream = false, trainingMode = false, webSearch = 'auto', memoryFromClient = false } = req.body || {};
    memoryFromClient = Boolean(memoryFromClient);
    ...
    if (wantsContext || think || isDeepEffort(effort) || trainingMode) {
      ...
      const [docsResult, memoriesResult, feedbackResult] = await Promise.all([
        ...
        memoryFromClient ? Promise.resolve([]) : fetchUserMemories(ownerId).catch(() => []),
