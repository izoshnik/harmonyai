...
.thinking.think-text{font-weight:600;letter-spacing:.01em;}
...
function createLiveTypingController(el){
  let pendingText='';
  let shownLength=0;
  let finalText=null;
  let timer=null;
  ...
  function tick(){
    ...
    if(shownLength<pendingText.length){
      ...
      shownLength+=remain>12?3:remain>4?2:1;
      const partial=pendingText.slice(0,shownLength);
      el.textContent=partial+(shownLength<pendingText.length||finalText===null?'▍':'');
      el.dataset.raw=partial;
      scrollChat();
  ...
  return {
    ...
    complete(text){
      if(typeof text==='string'&&text.length>pendingText.length)pendingText=text;
      finalText=typeof text==='string'&&text?text:pendingText;
      if(!timer)finishIfReady();
      return done;
    },
    done
  };
...
async function regenMsg(item){
  ...
  try{
    const finalText=await requestChatReplyLive(msgEl,actionsEl);
...
function showTyping(){
  ...
  if(thinkOn){
    ...
    d.textContent='Думать';
  }else{
    d.innerHTML='<div class="think-dots"><span></span><span></span><span></span></div><span style="font-size:14px;color:var(--text2)">Генерирую...</span>';
  }
  inner.appendChild(d);scrollChat();
}
...
async function requestChatReplyLive(msgEl,actionsEl){
  activeAbortController=new AbortController();
  const payload={
    messages:[{role:'system',content:buildSystem()},...chatHistory],
    model:curModel,
  ...
  for(let i=0;i<endpoints.length;i++){
    ...
    try{
      ...
      if(type.includes('text/event-stream')&&res.body){
        ...
        const typer=createLiveTypingController(msgEl);
        let buffer='';
        let gotDelta=false;
        while(true){
          ...
          for(const evt of events){
            ...
            for(const line of dataLines){
              ...
              if(payloadChunk.type==='delta'){
                if(!gotDelta){hideTyping();gotDelta=true;}
                typer.push(payloadChunk.text||'');
              }else if(payloadChunk.type==='done'){
                hideTyping();
                const finalText=await typer.complete(normalizeReplyNotation(payloadChunk.text||''));
        ...
        hideTyping();
        const finalText=await typer.complete(normalizeReplyNotation(msgEl.dataset.raw||msgEl.textContent||''));
        setMessageActionsVisible(actionsEl,true);
        return finalText;
      }
      let data=null;
...
async function sendMsg(){
  ...
  try{
    const finalText=await requestChatReplyLive(aiShell.msg,aiShell.actions);
    if(finalText)chatHistory.push({role:'assistant',content:finalText});
    await saveChat();
    pendingImg=null;
    pendingDoc=null;
