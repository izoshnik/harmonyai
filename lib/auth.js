...
/* Shared Pro entitlement: staff OR an active paid subscription.
   Explicit plan takes precedence over legacy role=pro. Expired/invalid dates fail closed. */
export function hasProAccess(profile, now = Date.now()) {
  if (!profile) return false;
  if (isUnlimitedApiRole(profile.role)) return true;
  const plan = String(profile.plan || '').trim().toLowerCase();
  const role = String(profile.role || '').trim().toLowerCase();
  if (plan !== 'pro' && !(plan === '' && role === 'pro')) return false;
  const expiry = profile.plan_expires_at;
  if (expiry == null || expiry === '') return true; // existing perpetual/legacy grants
  const until = Date.parse(expiry);
  return Number.isFinite(until) && until > now;
}
export async function readAccessProfile(userId) {
  if (!userId) return null;
  try {
    const rows = await supabaseRest(`/rest/v1/profiles?select=role,plan,plan_expires_at&id=eq.${encodeURIComponent(String(userId))}&limit=1`, {method:'GET'});
    return Array.isArray(rows) ? rows[0] || null : null;
  } catch (e) {
    console.warn('[auth] entitlement lookup failed');
    return null;
  }
}
