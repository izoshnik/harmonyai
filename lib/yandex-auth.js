...
export async function connectionStatus(userId) {
  ...
  return {
    ...
    available: true, // Enabled module, not presence of a token.
    canConnect: canStoreUserTokens(),
    uid: resolved.uid,login: resolved.login||null,
...
export async function requestDeviceCode() {
  if(!canStoreUserTokens())throw new YandexError('Подключение пока не настроено администратором',503,'not_configured');
  ...
  return {
    ...
    verificationUrl: data.verification_uri_complete || data.verification_url || data.verification_uri || 'https://ya.ru/device',
...
export async function pollDeviceToken(deviceCode, userId) {
  ...
  throw new YandexError('Не удалось завершить авторизацию. Начните подключение заново.',502,'oauth_error');
}
...
async function finalizeToken(token, userId) {
  ...
  const persisted = Boolean(userId && canStoreUserTokens());
  let warning = null;
  if (!persisted) {
    warning = !userId
      ? 'Не удалось определить пользователя — войдите в аккаунт HarmonyAI и повторите'
      : 'Сервер не может сохранить подключение: задайте MUSIC_ENCRYPTION_KEY и выполните supabase/music_accounts.sql';
  }
