...
export function normalizeTrack(track) {
  ...
  return {
    ...
    // Жанр берём из альбома: у самого трека его нет, а фильтру в плейлисте он нужен.
    genre: album && album.genre ? String(album.genre) : '',
...
/** Сводит название к сравнимому виду: регистр, ё, кавычки, пунктуация. */
...
/* Пометки версий, которые почти никогда не нужны, если о них не просили явно.
   Именно они давали «кучу одинаковых треков» в выдаче: караоке, каверы,
   ускоренные версии и сборники с тем же названием. */
const VERSION_JUNK = [
  'karaoke', 'караоке', 'cover', 'кавер', 'tribute', 'трибьют',
  'instrumental', 'инструментал', 'минус', 'backing track',
  'remix', 'ремикс', 'mashup', 'nightcore', 'sped up', 'speed up',
  'slowed', 'reverb', 'lo fi', 'lofi', 'remake', 'ремейк', 'parody', 'пароди',
  'in the style of', 'made famous by', 'made popular by', 'originally performed',
  'live', 'лайв', 'концертн', 'workout', 'колыбельн', 'lullaby',
];

/** Насколько трек соответствует запросу. Больше — лучше. */
function scoreTrack(track, query, position) {
  if (!track) return -Infinity;

  const q = foldTitle(query);
  const words = q.split(' ').filter((w) => w.length > 1);
  const title = foldTitle(track.title);
  const artist = foldTitle(track.artist);
  const hay = `${title} ${artist}`;

  // Позиция в выдаче Яндекса — слабый, но полезный сигнал.
  // Благодаря ему на размытых запросах («что-нибудь бодрое»)
  // порядок остаётся Яндексовым, а не случайным.
  let score = -Math.min(position, 20) * 1.5;

  if (title && q === title) score += 120;
  else if (title && q.includes(title)) score += 70;
  else if (title && title.includes(q)) score += 45;

  let hitTitle = 0;
  let hitArtist = 0;
  for (const w of words) {
    if (title.includes(w)) hitTitle += 1;
    else if (artist.includes(w)) hitArtist += 1;
  }
  if (words.length) {
    score += (hitTitle / words.length) * 60;
    score += (hitArtist / words.length) * 35;
  }

  // Караоке и каверы — только если их просили.
  for (const junk of VERSION_JUNK) {
    if (hay.includes(junk) && !q.includes(junk)) { score -= 55; break; }
  }

  if (track.playable === false) score -= 200;
  if (track.duration && track.duration < 45) score -= 25; // отрывки и джинглы

  return score;
}

/** Убирает повторы: одна и та же песня приходит из разных альбомов. */
function dedupeTracks(items) {
  const out = [];
  const seen = new Map();
  for (const t of items) {
    if (!t) continue;
    const key = `${foldTitle(t.title)}|${foldTitle(t.artist)}`;
    ...
    // Из двух одинаковых записей оставляем ту, что реально играется.
    const prev = out[idx];
    if (prev && prev.playable === false && t.playable !== false) out[idx] = t;
  }
  return out;
}
...
export async function searchTracks(query, { token, limit = 8 } = {}) {
  ...
  /* Раньше здесь было «берём best от Яндекса, иначе первый играбельный».
     Из-за этого наверх часто всплывало караоке или ускоренный кавер,
     а список состоял из десятка одной и той же песни из разных сборников.
     Теперь: склеили дубли и отсортировали по соответствию запросу. */
  const pool = dedupeTracks(value.best ? [value.best, ...value.items] : value.items);
  const ranked = pool
    .map((t, i) => ({ t, s: scoreTrack(t, q, i) }))
    .sort((a, b) => b.s - a.s)
    .map((x) => x.t);
...
/* Сколько треков запрашиваем за один раз: больше ставить нельзя — Яндекс
   обрезает слишком длинный track-ids и часть плейлиста просто пропадает. */
const TRACKS_CHUNK = 100;
...
export async function getTracks(ids, { token } = {}) {
  ...
  const { value } = await cacheWrap(
    key,
    async () => {
      /* Порядок восстанавливаем по исходному списку: плейлист не должен
         перемешиваться из-за того, в каком порядке ответил сервер. */
      const out = [];
      for (let i = 0; i < list.length; i += TRACKS_CHUNK) {
        const part = list.slice(i, i + TRACKS_CHUNK);
        const r = await ymFetch('/tracks', { token, method: 'POST', body: { 'track-ids': part.join(',') } });
        for (const raw of (Array.isArray(r) ? r : [])) {
          const n = normalizeTrack(raw);
          if (n) out.push(n);
        }
      }
      const byId = Object.create(null);
      for (const t of out) byId[t.trackId] = t;
      return list.map((id) => byId[String(id).split(':')[0]]).filter(Boolean);
    },
    TTL.TRACK,
    TTL.TRACK_STALE
  );
...
/**
 * Только идентификаторы лайкнутых треков.
 *
 * Нужен плееру, чтобы сердечко сразу показывало реальное состояние и совпадало
 * с приложением Яндекс.Музыки. В отличие от getLikedTracks не тянет метаданные
 * всей библиотеки — это один лёгкий запрос вместо двух тяжёлых.
 *
 * Кэш короткий: избранное пользователь меняет и из другого приложения.
 */
export async function getLikedTrackIds(uid, { token } = {}) {
  if (!uid) return [];
  const { value } = await cacheWrap(
    `likes:${uid}`,
    async () => {
      const r = await ymFetch(`/users/${encodeURIComponent(uid)}/likes/tracks`, { token });
      return (Array.isArray(r?.library?.tracks) ? r.library.tracks : [])
        .map((t) => String(t.id))
        .filter(Boolean);
    },
    60 * 1000,
...
/**
 * «Мне нравится» для экрана плейлиста: метаданные треков и сколько лайков всего.
 *
 * getLikedTracks отдаёт только срез и не говорит, сколько треков в библиотеке,
 * а подпись «216 треков» должна быть настоящей. Список id берётся из кэша
 * getLikedTrackIds, поэтому лишнего запроса к Яндексу здесь нет.
 */
export async function getLikedLibrary(uid, { token, limit = 100 } = {}) {
  if (!uid) return { items: [], total: 0 };
  const ids = await getLikedTrackIds(uid, { token });
  if (!ids.length) return { items: [], total: 0 };
  const items = await getTracks(ids.slice(0, Math.max(1, limit)), { token });
