...
/* HM-DATA-CONTROL: настройка «использовать мои чаты для улучшения сервиса».
   Хранится в profiles.settings (jsonb), по умолчанию включена. */
export async function handleTrainingSetting(req,res,user,body){
 if(req.method!=='POST')return fail(res,405,'Метод не поддерживается');
 if(typeof body.enabled!=='boolean')return fail(res,400,'Не указано значение enabled');
 try{
  const rows=await supabaseRest('/rest/v1/profiles?id=eq.'+encodeURIComponent(user.id)+'&select=settings&limit=1');
  const current=(rows&&rows[0]&&rows[0].settings&&typeof rows[0].settings==='object')?rows[0].settings:{};
  const settings={...current,ai_training:body.enabled};
  await supabaseRest('/rest/v1/profiles?id=eq.'+encodeURIComponent(user.id),{method:'PATCH',body:JSON.stringify({settings})});
  return res.status(200).json({ok:true,enabled:body.enabled});
 }catch(e){return fail(res,503,'Не удалось сохранить настройку. Попробуйте ещё раз позже.');}
}
