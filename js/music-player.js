...
(function (global) {
  ...
  function bindEvents() {
    ...
    /* Уход со страницы или сворачивание вкладки — фиксируем позицию сразу,
       ...
       persisted === false — это настоящий уход с сайта (закрытие вкладки,
       переход по внешней ссылке, перезагрузка): останавливаем звук, чтобы
       плеер точно не «переехал» на следующую страницу. persisted === true —
       это BFCache (переключение вкладок/приложений на телефоне), там звук
       обязан продолжать играть: фоновое воспроизведение с управлением на
       локскрине живёт именно на этом. */
    global.addEventListener('pagehide', function (ev) {
      writeState();
      if (!ev || !ev.persisted) pause();
    });
  ...
  /**
   * Восстанавливаем очередь, НО НЕ начинаем играть и НЕ показываем панель.
   *
   * Панель специально не открывается: уход с сайта = закрытый плеер. Раньше
   * здесь вызывался show(), и мини-плеер сам возвращался на экран при каждом
   * заходе — даже если человек его закрыл или просто ушёл со страницы.
   * Очередь и позицию храним молча: если пользователь снова нажмёт Play (из
   * плейлиста, с клавиатуры или из ответа ассистента), playAt() покажет
   * панель сам и продолжит с того же места.
   */
  function restore() {
    var raw;
