/* ============================================================================
   ...
   А вот если глагол есть («найди», «включи»), фраза разбирается ДАЖЕ когда
...
(function (global) {
  ...
  /* Признаки запроса, который ГАРАНТИРОВАННО не про музыку: поиск в интернете,
     инструкции, бытовая справка. Проверяются раньше глаголов, потому что
     «найди» одинаково начинает и «найди трек Believer», и «найди в интернете,
     как получить иностранную карту». Второе уходило в поиск по Яндекс.Музыке и
     возвращало «Не нашёл ничего по запросу…» вместо ответа по существу. */
  var WEB_REQUEST = [
    stemRe('в интернете|в инете|в интернет|в сети|загугли|гугли|нагугли|в гугле|в яндексе|в браузере|на сайте|веб-поиск'),
    /\b(google|internet|web\s+search)\b/i,
    stemRe('как\\s+(?:получить|оформить|сделать|создать|открыть|узнать|заработать|купить|заказать|установить|настроить|подключить|зарегистр|перевести|попасть|добраться|вернуть|восстановить)'),
    stemRe('где\\s+(?:купить|заказать|найти|оформить|получить)|сколько\\s+стоит|во\\s+сколько\\s+обойд'),
    wordRe('инструкцию|инструкция|информацию|инфу|новости|погоду|погода|рецепт|адрес|телефон|цену|стоимость|курс|документы|справку|визу|карту|счет|налог|паспорт')
  ];

  function webRequest(text) {
    for (var i = 0; i < WEB_REQUEST.length; i++) {
      if (WEB_REQUEST[i].test(text)) return true;
    }
    return false;
  }
  ...
  function blocked(text) {
    if (webRequest(text)) return true;
  ...
  var QUESTION_ALTS = ''
    ...
    /* Инструкция, а не музыка: «найди, как получить карту», «найди, как оформить визу». */
    + 'как\\s+(?:получить|оформить|сделать|создать|открыть|узнать|заработать|купить|заказать|установить|настроить|подключить|зарегистрироваться|восстановить|вернуть|попасть|добраться)|'
  ...
  /* Мусор в начале запроса и вежливый хвост.
     Предлоги сюда намеренно НЕ добавлены: «поставь плейлист для бега» и
     «включи В лесу родилась ёлочка» потеряли бы часть запроса. Веб-запросы
     отсекаются раньше — гейтом webRequest() и списком QUERY_BAD. */
  ...
  /* Остаток, который выглядит как учебный или веб-запрос, а не как название записи. */
  var QUERY_BAD = [
    ...
    stemRe('интернет|инете|загугл|гугл|браузер|сайт|ссылк|инструкц|документ|справк|налог|паспорт|виз[уы]'),
    /\b(google|internet|website|url|link)\b/i
  ];
   ...
   * @returns {null|{kind:string,kindFound:boolean,query:string,wantAnswer:boolean,answerRequest:string,random:boolean}}
   */
  function parseObject(rest) {
    ...
    if (RANDOM_RE.test(text)) {
      return { kind: 'track', kindFound: false, query: '', wantAnswer: false, answerRequest: '', random: true };
    }
    ...
    if (RANDOM_RE.test(query)) {
      return { kind: kind, kindFound: kindFound, query: '', wantAnswer: wantAnswer, answerRequest: '', random: true };
    }
  ...
  var PLAY_VERB = stemRe('включ|поставь|поставить|запусти|запуст|врубай|врубани|врубить|сыграй|играй|включай|дай послушать|хочу послушать|хочется послушать|замути|заряди');
  var PLAY_VERB_EN = /\b(play|turn on|put on|queue)\b/i;
  ...
  /** Стоит ли будить серверный LLM-классификатор.
      ...
      Порядок проверок важен: веб-запрос отсекается ДО глаголов, иначе «найди
      в интернете…» попадало сюда как музыкальное. Глагол воспроизведения,
      наоборот, сильнее стоп-фильтров: «включи что-нибудь и расскажи про трек»
      — это музыка со второй просьбой, а не учебный вопрос. */
  function looksMusicRelated(rawText) {
    ...
    if (webRequest(norm)) return false;
    if (PLAY_VERB.test(norm) || PLAY_VERB_EN.test(norm)) return true;
    if (blocked(norm)) return false;
    if (MUSIC_SURFACE.test(norm) || MUSIC_SURFACE_EN.test(norm)) return true;
    if (ACTION_VERB.test(norm) || ACTION_VERB_EN.test(norm)) return Boolean(bareName(norm));
  ...
  function detect(rawText) {
    ...
    if (mode) {
      /* Веб-запрос («найди в интернете как получить карту») никогда не музыка,
         даже с глаголом поиска — отдаём обычному чату, там есть веб-поиск. */
      if (webRequest(norm)) return null;
      ...
      /* Музыкальный признак = глагол воспроизведения («включи/поставь/врубай»)
         ЛИБО музыкальное существительное («трек», «альбом», «песню», «плейлист»).
         Один «найди» таким признаком не является: он одинаково начинает
         «найди трек Believer» и «найди рецепт борща». Без признака решение
         принимает серверный классификатор (confidence:'low'), а не регэксп. */
      var musical = mode === 'play'
        || parsed.kindFound
        || MUSIC_SURFACE.test(norm)
        || MUSIC_SURFACE_EN.test(norm);
      ...
      return {
        ...
        confidence: (musical && !parsed.wantAnswer) ? 'high' : 'low',
        autoplay: mode === 'play',
        wantAnswer: parsed.wantAnswer,
        answerRequest: parsed.answerRequest,
        /* explicit — «локального разбора достаточно, чтобы играть без модели».
           Только явная музыкальная команда: она должна работать и без сети. */
        explicit: musical
      };
    }
